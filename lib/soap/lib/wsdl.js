...
function splitNSName(nsName) {
    // XXX modif lib: patch split namespace
    var i = (nsName != null && typeof nsName == 'string') ? nsName.indexOf(':') : -1;
...
DefinitionsElement.prototype.addChild = function(child) {
    ...
    if (child instanceof TypesElement) {
      ...
      // XXX lib modification, don't assert for PROPERTY child type
...
WSDL.prototype.xmlToObject = function(xml) {
    ...
    p.on('startElement', function(nsName, attrs) {
        ...
        stack.push({name: originalName, object: obj, schema: topSchema && topSchema[name], id:attrs.id, attrs: attrs});
    })
    ...
    p.on('endElement', function(nsName) {
        var cur = stack.pop(),
            ...
            // XXX modif lib: attributes
            attrs = cur.attrs;

        // XXX modif lib: fix simple values interpreted as array
        if (topSchema && topSchema[name+'[]'] && typeof obj == 'array') {
            ...
            // XXX modif lib: if empty object -> empty string
        ...
        // XXX modif lib: add JSON attrs
        if (topSchema != undefined
          && topSchema['Envelope'] == undefined
          && topSchema['Header'] == undefined) {

          if(JSON.stringify(attrs) != '{}') {
            if(topObject['@'+ name]) {
                topObject['@'+ name] = [topObject['@'+ name]];
                topObject['@'+ name].push(attrs);
            }
            else
                topObject['@'+ name] = attrs;
          }
        }
        //
    ...
    p.on('text', function(text) {
        ...
        // XXX modif lib, default value for name
...
//            console.log('INT', tex)
    ...
    // if problem then do nothing
    if(!root || !root.Envelope) {
      return null;
    }
    ...
    if(!exports.WSDL.WITH_ATTR_AT && exports.WSDL.PROTOCOL_VERSION == '1.6') {
        ...
        if(cmd == 'meterValuesRequest' || cmd == 'stopTransactionRequest') {
            ...
            if(cmd == 'stopTransactionRequest') {
                values = root.Envelope.Body[cmd].transactionData;
            }

            for(var i in values) {
                var valuesField;
                if(cmd == 'stopTransactionRequest') {
                    valuesField = values[i]['values'];
                }
                else if(cmd == 'meterValuesRequest') {
                    valuesField = values[i];
                }

                if(valuesField['value'] instanceof Object) {
                    var attrs = {};

                    attrs['value'] = valuesField['value'][0];
                    for(var attr in valuesField['@value'][0]) {
                        attrs[deleteNamespace(attr)] = valuesField['@value'][0][attr];
                    }

                    valuesField['value'] = attrs;
                    delete valuesField['@value'];
                }
                if(valuesField['value'] instanceof Array) {
                    valuesField['values'] = [];

                    for(var v in valuesField['value']) {
                        var attrs = {};

                        attrs['value'] = valuesField['value'][v];
                        for(var attr in valuesField['@value'][v]) {
                            attrs[deleteNamespace(attr)] = valuesField['@value'][v][attr];
                        }

                        valuesField['values'].push(attrs);
                    }
...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
    var self = this,
        ...
        nsAttrName = '_xmlns';
        procName = name.charAt(0).toLowerCase() + name.slice(1);
    parts.push(['<',namespace,':',procName,'>'].join(''));
    for (var key in params) {
        if (key != nsAttrName) {
            var value = params[key];
            parts.push(['<tns:',key,'>'].join(''));
            parts.push((typeof value==='object')?this.objectToXML(value, null, 'tns'):xmlEscape(value));            
            parts.push(['</tns:',key,'>'].join(''));
        }
    }
    parts.push(['</',namespace,':',procName,'>'].join(''));
...
/* XXX modif lib : SOAP-ENV:mustUnderstand */
WSDL.prototype.objectToXML = function(_obj, name, namespace, xmlns, mustUnderstand, attributes) {
    var self = this,
        parts = [],
        xmlnsAttrib = false ? ' xmlns:'+namespace+'="'+xmlns+'"'+' xmlns="'+xmlns+'"' : '',
        ns = namespace ? namespace + ':' : '',
        mustUnderstand = mustUnderstand || false,
        mustUnderstandAttr = mustUnderstand ? ' soap:mustUnderstand="true"' : '' ;
    
    // must work on a copy
    var obj = clone(_obj);

    var addAttributes = function(name, attrs, ns) {
        var str = '';

        if(name == 'value' && attrs != undefined && attrs.length > 0) {
          var attr = attrs.splice(0, 1);
          attr = attr[0];
          for(var a in attr) {
            str += ' '+ ns + a +'="'+ attr[a] +'"';
          }
    ...
    if (Array.isArray(obj)) {
        for (var i=0, item; item=obj[i]; i++) {
            if (i > 0) {
                ...
                var attrs = addAttributes(name, attributes, ns);
            ...
            for (var name in obj) {
                if(name == 'values' && !obj.values[0].values) {
                    ...
                    if(obj.values.length == 1) {
                        ...
                        delete obj.values[0].value;
                        obj['@value'] = clone(obj.values[0]);
                        ...
                        for(var i in obj.values) {
                            obj['value'].push(obj.values[i].value);
                            delete obj.values[i].value;
                            obj['@value'].push(clone(obj.values[i]));
                        }
...
// XXX end
            ...
            var attrs = obj['@value'] || null;

            if(attrs) {
                attrs_str = addAttributes(name, obj['@value'], ns);
            }

            parts.push(['<',ns,name,xmlnsAttrib,mustUnderstandAttr,attrs_str,'>'].join(''));
            // XXX: namespace in nested node
            parts.push(self.objectToXML(child, name, namespace, xmlns, false, attrs));
...
//          alias = 'ocppCs12';
    ...
    // XXX modif lib : add wsa5 namespace
    str += ' xmlns:wsa5="http://www.w3.org/2005/08/addressing"';
...
exports.WSDL = WSDL;
// XXX
exports.WSDL.WITH_ATTR_AT = false;
exports.WSDL.PROTOCOL_VERSION = null;
